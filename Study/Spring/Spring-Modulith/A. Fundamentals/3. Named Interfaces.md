...
prev: "[[2-5. Explicit Application Module Dependencies]]"
next: "[[4. Customizing the Application Modules Arrangement]]"
---
...
> [!IMPORTANT]
> **이름은 자유롭게 지어도 되요**
> `"spi"`는 공식 문서의 예시일 뿐이에요.  이 어노테이션의 목적은 "이 하위 폴더(패키지)에 내가 원하는 이름표를 붙이는 것"이에요.
> 
> ** 예시**
...
### Named Interface 호출
`Named Interface`는 다른 외부 모듈(`inventory`)에서 호출할 때 다음과 같은 방법으로 설정할 수 있어요.
...
이외에도 명명된 인터페이스에 대해 더 복잡하고 일반적인 제어가 필요하다면 
**커스텀 네임드 인터페이스 탐색 옵션 설정을 활용할 수 있답니다.**
